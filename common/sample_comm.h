...
#define nalu_sent_len        1400			// 每包要发送的数据大小
...
typedef struct
{
	...
	int seqnum;			// 序列号
	int seqnum2;
	unsigned int tsvid;	// 计算timestamp的值
	unsigned int tsaud;
	int status;
	int sessionid;
	int rtpport[2];
...
typedef struct _rtpbuf
{
	...
	HI_S32 len;		// 这个是告诉有效数据的长度；
	// 指向的是一个字符串，也不知道字符串有多长，所以需要一个len来告诉我们有效数据的长度；
	char * buf;		// 这个指针是指向的地址是用来真正存储有效数据的；
}RTPbuf_s;
