...
{
	// 编码图像最大宽度，取值范围：[MIN_WIDTH, MAX_WIDTH]，以像素为单位。
	// 必须是 MIN_ALIGN 的整数倍。静态属性。
    HI_U32  u32MaxPicWidth;                         /*maximum width of a picture to be encoded, in pixel*/
	// 编码图像最大高度，取值范围：[MIN_HEIGHT, MAX_HEIGHT]，以像素为单位。
	// 必须是 MIN_ALIGN 的整数倍。静态属性。
	HI_U32  u32MaxPicHeight;                        /*maximum height of a picture to be encoded, in pixel*/

	// 码流buffer大小。取值范围：[Min, Max]，以 byte 为单位。必须是64的整数倍。
	// 推荐值：一幅最大编码图像大小。推荐值为 u32MaxPicWidth×u32MaxPicHeight ×1.5 byte。静态属性。
	// 加载ko时设置模块参数H264eMiniBufMode可以选择取值范围限制方式。
	// 1、H264eMiniBufMode = 0：Min-一幅最大编码图像大小的1/2。Max-无限制，但是会消耗更多的内存
	// 2、H264eMiniBufMode = 1，用户保证buffer 大小合理性：Min：32x1024 byte；Max：无限制，但是会消耗更多的内存。
    HI_U32  u32BufSize;                             /*stream buffer size*/
	// 编码的等级。取值范围：[0, 3]。0：Baseline。1：Main Profile。2：High Profile。3：svc-t。
	HI_U32  u32Profile;                             /*0: baseline; 1:MP; 2:HP; 3: SVC-T [0,3]; */          
	// 帧/包模式获取码流。取值范围：{HI_TRUE, HI_FALSE}。HI_TRUE：按帧获取。HI_FALSE：按包获取。静态属性
	HI_BOOL bByFrame;                               /*get stream mode is slice mode or frame mode*/
	

	// 编码图像宽度。取值范围：[MIN_WIDTH, MAX_WIDTH]，以像素为单位。
	// 必须是 MIN_ALIGN 的整数倍。静态属性。
    HI_U32  u32PicWidth;                            /*width of a picture to be encoded, in pixel*/
	// 编码图像高度。取值范围：[MIN_HEIGHT, MAX_HEIGHT]，以像素为单位。
	// 必须是 MIN_ALIGN 的整数倍。静态属性。
	HI_U32  u32PicHeight;                           /*height of a picture to be encoded, in pixel*/    

	// 编码支持B帧的个数。取值范围：[0, Max]。Max：无限制。HI3516A/Hi3518EV200中u32BFrameNum 为保留字段，暂不支持。
    HI_U32  u32BFrameNum;                           /* 0: not support B frame; >=1: number of B frames */
	// 编码支持参考帧的个数。取值范围：[1, 2]。HI3516A/Hi3518EV200中u32BFrameNum 为保留字段，暂不支持。
