...
{
	// H.264 gop 值。取值范围：[1, 65536]。
    HI_U32      u32Gop;                                 /*the interval of ISLICE. */
	// CBR 码率统计时间，以秒为单位。取值范围：[1, 60]。
    HI_U32      u32StatTime;                            /* the rate statistic time, the unit is senconds(s) */
	// VI 输入帧率，以fps为单位。取值范围：[1, 240]。
	HI_U32      u32SrcFrmRate;                          /* the input frame rate of the venc chnnel */
	// 编码器输出帧率，以fps为单位。取值范围：(0, u32SrcFrmRate]。
	HI_FR32     fr32DstFrmRate ;                        /* the target frame rate of the venc chnnel */ 
	// 平均 bitrate，以kbps为单位。取值范围：[2, 102400]。
	HI_U32      u32BitRate;                             /* average bitrate */
	// 最大码率相对平均码率的波动等级，保留，暂不使用。取值范围：[1, 5]。推荐使用波动等级 1。
...
typedef struct hiVENC_ATTR_H264_VBR_S
{
	// H.264 gop 值。取值范围： [1, 65536]。
    HI_U32      u32Gop;                                 /*the interval of ISLICE. */
	// VBR 码率统计时间，以秒为单位。取值范围： [1, 60]。
	HI_U32      u32StatTime;                            /* the rate statistic time, the unit is senconds(s) */
	// VI 输入帧率，以 fps 为单位。取值范围： [1, 240]。
	HI_U32      u32SrcFrmRate;                          /* the input frame rate of the venc chnnel */
	// 编码器输出帧率，以 fps 为单位。取值范围： (0, u32SrcFrmRate]。
	HI_FR32     fr32DstFrmRate ;                        /* the target frame rate of the venc chnnel */     
	// 编码器输出最大码率，以 kbps 为单位取值范围： [2, 102400]。
	HI_U32      u32MaxBitRate;                          /* the max bitrate */                      
	// 编码器支持图像最大 QP。取值范围： (u32MinQp, 51]。
	HI_U32      u32MaxQp;                               /* the max qp */
	// 编码器支持图像最小 QP。取值范围： [0, 51]。
