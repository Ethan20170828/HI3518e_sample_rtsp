...
{
	// PAL(phase alteration line)制式，意思是逐行倒相，也属于同时制；
	// PAL制式是中国、欧洲等国家和地区的电视广播中色彩编码的一种方法；
	// 如果在信号传输过程中发生相位失真，则会由于相邻两行信号的相位相反起到互相补偿作用，
	// 从而有效地克服了因相位失真而起的色彩变化。
    VIDEO_ENCODING_MODE_PAL = 0,
   	// NTSC(national television standards committee)制式，用于美国、日本、加拿大等国家和地区
...
typedef enum hiPIXEL_FORMAT_E
{
    ...
    PIXEL_FORMAT_RGB_BAYER,         /* 16 bpp rawRGB*/
...
// 宽动态的模式，就是在非常强烈的对比下让摄像机看到影像的特色而运用的一种技术
