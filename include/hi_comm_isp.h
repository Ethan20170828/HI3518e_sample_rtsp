...
// 定义输入Bayer图像数据格式，Bayer格式图片在一块滤镜上设置的不同的颜色，通过分析人眼
// 对颜色的感知发现，人眼对绿色比较敏感，所以一般Bayer格式的图片绿色格式的像素是r和g像素的和。
// Bayer是相机内部的原始图片，一般后缀名为.raw
typedef enum hiISP_BAYER_FORMAT_E
{
    BAYER_RGGB    = 0,	// RGGB排列方式
    BAYER_GRBG    = 1,	// GRGB排列方式
    BAYER_GBRG    = 2,	// GBRG排列方式
    BAYER_BGGR    = 3,	// BGGR排列方式
    BAYER_BUTT    
} ISP_BAYER_FORMAT_E;
...
{
    RECT_S          stWndRect;      /* RW. 裁剪窗口起始位置和图像宽高*/
    HI_FLOAT        f32FrameRate;   /* RW. 输入图像帧率，取值范围为(0.00, 255.00)*/
    ISP_BAYER_FORMAT_E  enBayer;    /* RW. Bayer数据格式*/
} ISP_PUB_ATTR_S;
