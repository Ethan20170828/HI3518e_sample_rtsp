...
{
	// 输入数据的协议符合标准BT.656协议，端口数据输入模式为亮度色度复合模式，分量模式为单分量；
    VI_MODE_BT656 = 0,              /* ITU-R BT.656 YUV4:2:2 */
    // 输入数据的协议符合标准BT601协议，端口数据输入模式为亮度色度复合模式，分量模式为单分量；
    ...
    // 输入数据的协议符合BT1120协议(BT656+双分量)，端口数据输入模式为亮度色度分离模式，分量模式为双分量；
    VI_MODE_BT1120_STANDARD,        /* BT.1120 progressive mode */
    // 输入数据的协议符合BT1120 interleave模式，端口数据输入模式为亮度色度分离模式，分量模式为双分量；
...
typedef enum hiVI_WORK_MODE_E
{
    ...
    // 2路复合工作模式，输入数据的协议必须为标准BT656协议；
    VI_WORK_MODE_2Multiplex,        /* 2 Multiplex mode */
    // 4路复合工作模式，输入数据的协议必须为标准BT656协议
...
{
	// 垂直同步翻转模式，即翻转一次表示一场。BT601模式下表示场号，DC模式下表示行有效信号；
...
{
	// 若 VI_VSYNC_E = VI_VSYNC_FIELD，则表示偶数场的vsync信号为高电平；
	// 若 VI_VSYNC_E = VI_VSYNC_PULSE，则表示 vsync 同步脉冲为正脉冲；
    ...
    // 若 VI_VSYNC_E = VI_VSYNC_FIELD，则表示偶数场的vsync信号为低电平；
    // 若 VI_VSYNC_E = VI_VSYNC_PULSE，则表示 vsync 同步脉冲为负脉冲；
...
typedef enum hiVI_DATA_TYPE_E
{
    ...
    VI_DATA_TYPE_RGB = 1,	// 输入数据类型为RGB，VI前端一般接的是sensor；
    VI_DATA_TYPE_BUTT
} VI_DATA_TYPE_E;

// VI输入数据通路枚举
// 注意: (1)YUV、带ISP的sensor输入时，必须配置为VI_PATH_BYPASS；
//		 (2)不带ISP的sensor输入时，配置为VI_PATH_ISP；
typedef enum hiVI_DATA_PATH_E
{
    VI_PATH_BYPASS    = 0,        /* ISP bypass ；VI内部ISP Bypass*/
    VI_PATH_ISP       = 1,        /* ISP enable ；VI内部ISP Enable*/
    VI_PATH_RAW       = 2,        /* Capture raw data, for debug ；保留类型，不支持*/
    VI_PATH_BUTT
}VI_DATA_PATH_E;
...
/* the attributes of a VI device ；结构体作用: 定义视频输入设备的属性*/
typedef struct hiVI_DEV_ATTR_S
{
    VI_INTF_MODE_E      enIntfMode;         /* Interface mode 接口模式*/
	// 1/2/4路复合工作模式
    VI_WORK_MODE_E      enWorkMode;         /*1-, 2-, or 4-channel multiplexed work mode */

	// 分量掩码配置。当enIntfMode=VI_MODE_BT1120_STANDARD时，需要配置Y和C的分量掩码，其他模式时
	// 只需配置单分量掩码；
    HI_U32              au32CompMask[2];    /* Component mask */
	// 输入扫描模式(逐行、隔行)
    VI_SCAN_MODE_E      enScanMode;         /* Input scanning mode (progressive or interlaced) */
	// 取值范围[-1,3]，推荐统一设置为默认值-1
    HI_S32              s32AdChnId[4];      /* AD channel ID. Typically, the default value -1 is recommended */

    /* The below members must be configured in BT.601 mode or DC mode and are invalid in other modes */
	// 输入数据顺序(仅支持YUV格式)，BT601和DC模式时必须配置，其他模式时无效；
	VI_DATA_YUV_SEQ_E   enDataSeq;          /* Input data sequence (only the YUV format is supported) */
	// 同步时序配置，BT601和DC模式时必须配置，其他模式时无效；
	VI_SYNC_CFG_S       stSynCfg;           /* Sync timing. This member must be configured in BT.601 mode or DC mode */

	// 输入数据通路配置，不带ISP的sensor输入时，配置为ISP通路；带ISP的sensor或者AD输入时，配置为
	// BYPASS通路(VI内置ISP将Disable)；RAW Data一般仅在Debug时使用，用于捕获sensor输入的原始数据；
    VI_DATA_PATH_E      enDataPath;         /* ISP enable or bypass */
	// 输入数据类型，sensor输入一般为RGB，AD输入一般为YUV；
    VI_DATA_TYPE_E      enInputDataType;    /* RGB: CSC-709 or CSC-601, PT YUV444 disable; YUV: default yuv CSC coef PT YUV444 enable. */
	
	// 因为走线约束等硬件原因，有可能出现AD/sensor的数据线与VI数据线连接数据高低位反接，比如，AD_DATA0 与
	// VIU_DATA7 连接， AD_DATA7 与 VIU_DATA0 连接，以此类推。当AD/sensor管脚与VI管脚正向连接时，取
	// bDataRev=HI_FALSE；当反向连接时，取 bDataRev= HI_TRUE；
    HI_BOOL             bDataRev;           /* Data reverse */

	// VI设备可设置要捕获图像的起始位置、高度、从原始视频中裁剪出部分图像(PORT处CROP，前端裁剪，可以
	// 减小WDR/FPN的图像写出，节省一些带宽和内存)；
	// 捕获图像的最小宽高与最大宽高：
	// 宽度： [VIU_DEV_MIN_WIDTH， VIU_DEV_MAX_WIDTH]
	// 高度： [VIU_DEV_MIN_HEIGHT， VIU_DEV_MAX_HEIGHT]
    RECT_S              stDevRect;          /* Dev capture rect */
} VI_DEV_ATTR_S;
...
{
    VI_CAPSEL_TOP = 0,                  /* top field ；选择顶场*/
    VI_CAPSEL_BOTTOM,                   /* bottom field ；选择底场*/
    VI_CAPSEL_BOTH,                     /* top and bottom field ；选择顶底两场*/
    VI_CAPSEL_BUTT
} VI_CAPSEL_E;
...
{
	// 捕获区域起始坐标(相对于设备图像的大小)与宽高；
	// 捕获区域的最小宽高与最大宽高；
	// 宽度 [VIU_CHN_MIN_WIDTH， VIU_CHN_MAX_WIDTH]
	// 高度 [VIU_CHN_MIN_HEIGHT， VIU_CHN_MAX_HEIGHT]
	...
	// 帧场选择，只用于隔行模式，建议捕获单场时选择捕获底场；逐行模式时，该项必须设置为VI_CAPSEL_BOTH。
...
typedef struct hiVI_ISP_WDR_ATTR_S
{
    ...
    HI_BOOL bCompress;		// 是否压缩，HI_FALSE 为不压缩， HI_TRUE 为压缩；
}VI_WDR_ATTR_S;
